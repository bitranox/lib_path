...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : /usr/local/bin/lib_path --version"
  if ! "/usr/local/bin/lib_path" --version; then
...
function test_commandline_interface_venv_old() {
  ...
  clr_green "issuing command : $HOME/venv/local/bin/lib_path --version"
  if ! "$HOME/venv/local/bin/lib_path" --version; then
    my_banner_warning "test commandline interface on virtual environment ERROR"
    beep
    sleep "${sleeptime_on_error}"
    return 1
